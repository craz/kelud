...
?>
    ...
    echo $arResult["FORM_ACTION"] ?>" class="market__filters smartfilter" method="get">
        <? foreach ($arResult["HIDDEN"] as $arItem): ?>
            <input type="hidden" name="<? echo $arItem["CONTROL_NAME"] ?>" id="<? echo $arItem["CONTROL_ID"] ?>"
                   value="<? echo $arItem["HTML_VALUE"] ?>"/>
        <? endforeach; ?>
        <input type="hidden" name="sort_by" value="property_IN_STOCK" onchange="smartFilter.keyup(this)">
        <input type="hidden" name="sort_order" value="ASC" onchange="smartFilter.keyup(this)">

        <?
        foreach ($arResult["ITEMS"] as $item) {

			$minid = $item["VALUES"]["MIN"]['CONTROL_ID'];				
			if (isset($_GET[$minid])) {
				?>
				<input type="hidden" name="get_<?=$minid?>" value="<?=$_GET[$minid]?>">
				<?
			}
			
			$maxid = $item["VALUES"]["MAX"]['CONTROL_ID'];				
			if (isset($_GET[$maxid])) {
				?>
				<input type="hidden" name="get_<?=$maxid?>" value="<?=$_GET[$maxid]?>">
				<?
			}
            ...
            switch ($item["DISPLAY_TYPE"]) {
                case "A": //NUMBERS_WITH_SLIDER
                    {
                        if ($item["VALUES"]["MIN"]["VALUE"] == $item["VALUES"]["MAX"]["VALUE"] || $item["VALUES"]["MIN"]["VALUE"] == NULL || $item["VALUES"]["MAX"]["VALUE"] == NULL) {
                            break;
                        }
                        if ($item["CODE"] == "PRICE") {
                            ?>
                            <div class="market__filter m-filter filter-price">
                                <h3 class="m-filter__title"><?= $item["NAME"] ?></h3>
                                <div class="m-filter__slider m-slider">
                                    <div class="m-slider__wrapper">
                                        <div class="m-slider__item">
                                            <div class="m-slider__from">от <span></span></div>
                                            <input type="hidden" name="<?
                                            echo $item["VALUES"]["MIN"]["CONTROL_NAME"] ?>"
                                                   id="<?
                                                   echo $item["VALUES"]["MIN"]["CONTROL_ID"] ?>"
                                                   value="<?
                                                   echo $item["VALUES"]["MIN"]["VALUE"] ?>"

                                                   onchange="smartFilter.keyup(this)"
                                            >
                                        </div>
                                        <div class="m-slider__item">
                                            <div class="m-slider__to">до <span></span></div>
                                            <input type="hidden" name="<?
                                            echo $item["VALUES"]["MAX"]["CONTROL_NAME"] ?>"
                                                   id="<?
                                                   echo $item["VALUES"]["MAX"]["CONTROL_ID"] ?>"
                                                   value="<?
                                                   echo $item["VALUES"]["MAX"]["VALUE"] ?>"
                                    ...
                                    <?
                                    $min = (int)$item["VALUES"]["MIN"]["HTML_VALUE"];
                                    if ($min == 0) {
                                        $min = (int)$item["VALUES"]["MIN"]["VALUE"];
                                    }
                                    $max = (int)$item["VALUES"]["MAX"]["HTML_VALUE"];
                                    if ($max == 0) {
                                        $max = (int)$item["VALUES"]["MAX"]["VALUE"];
                                    }

                                    ?>

                                    <div class="m-slider__slider" data-min="<?= (int)$item["VALUES"]["MIN"]["VALUE"] ?>"
                                         data-min-value="<?= $min ?>"
                                         data-max="<?= (int)$item["VALUES"]["MAX"]["VALUE"] ?>"
                                         data-max-value="<?= $max ?>"
                                         data-step="10000"></div>
                            ...
                            <div class="market__filter m-filter">
                                ...
                                <div class="m-filter__slider m-slider">
                                    <div class="m-slider__wrapper">
                                        <div class="m-slider__item">
                                            <div class="m-slider__from">от <span></span> см</div>
                                            <input type="hidden" name="<?
                                            echo $item["VALUES"]["MIN"]["CONTROL_NAME"] ?>"
                                                   id="<?
                                                   echo $item["VALUES"]["MIN"]["CONTROL_ID"] ?>"
                                        ...
                                        <div class="m-slider__item">
                                            <div class="m-slider__to">до <span></span> см</div>
                                            <input type="hidden" name="<?
                                            echo $item["VALUES"]["MAX"]["CONTROL_NAME"] ?>"
                                                   id="<?
                                                   echo $item["VALUES"]["MAX"]["CONTROL_ID"] ?>"
                ...
                case "F":
                    {
                        ...
                        if (!$disableColor && $item["CODE"] == "COLOR") {
                            ?>
                            <div class="market__filter m-filter">
                                <h3 class="m-filter__title"><?= $item["NAME"] ?></h3>
                                <div class="m-colors">
                                    ...
                                    foreach ($item["VALUES"] as $value) {
                                        ?>
                                        <label class="m-colors__item <? echo $value["CHECKED"] ? 'm-colors__item_active' : '' ?> "
                                               style="background-color: <?= $value["SUBINFO"][0]["UF_COLOR_CODE"] ?>;">
                                            <input class="m-colors__input visuallyhidden" type="checkbox"
                            ...
                            <?
                        } elseif ($item["CODE"] == "IN_STOCK") { ?>
                            <div class="market__filter m-filter">
                                <label class="m-filter__item">
                                    <input type="checkbox" class="m-filter__checkbox"
                                           value="<?= $item["VALUES"][1]["HTML_VALUE"] ?>"
                                           name="<?= $item["VALUES"][1]["CONTROL_NAME"] ?>"
                                           id="<?= $item["VALUES"][1]["CONTROL_ID"] ?>"
                                           onclick="smartFilter.click(this)"
                                        <? echo $item["VALUES"][1]["CHECKED"] ? 'checked="checked"' : '' ?>
                                    >
                                    <div class="m-filter__square-checkbox">
                                        <svg class="m-filter__check-icon">
                        ...
                        <? } elseif ($item["CODE"] == "EDGE" || $item["CODE"] == "COLLECTION") {
                            ...
                            <div class="market__filter m-filter">
                                ...
                                foreach ($item['VALUES'] as $value) { ?>
                                    <label class="m-filter__item">
                                        <input type="checkbox" class="m-filter__checkbox" name="<?= $value["CONTROL_NAME"] ?>"
                                            <? echo $value["CHECKED"] ? 'checked="checked"' : '' ?>
                                               id="<?= $value["CONTROL_ID"] ?>" onclick="smartFilter.click(this)"
		...
		<div id="block-filter">
			<button id="btn-filter" class="detailed__buy button button_gold"><?= GetMessage("CT_BCSF_SET_FILTER") ?></button>
			<button id="btn-filter-reset" class="button"><?= GetMessage("CT_BCSF_DEL_FILTER") ?></button>
		</div>
		<style>
			#block-filter
			{
