...
$arResult["PROPERTIES"]["PRICE"]["VALUE"] = number_format($arResult["PROPERTIES"]["PRICE"]["VALUE"], 0, "", " ");
...
$arFilter = Array(
    "IBLOCK_ID" => $arResult["IBLOCK_ID"],
    "ACTIVE" => "Y",
    "ID" => $arResult["PROPERTIES"]["PODSTOLIE"]["VALUE"],
);


$arr_podstols = CIBlockElement::GetList(Array(""), $arFilter, false, false, $arSelect);
$allPodstol = [];

while ($arr_podstol = $arr_podstols->GetNext()) {
    if (CFile::GetPath($arr_podstol["DETAIL_PICTURE"])) {


        $arr_podstol["PREVIEW_PICTURE"] = CFile::GetFileArray($arr_podstol["DETAIL_PICTURE"]);
        $arr_podstol["PREVIEW_PICTURE"] = CFile::ResizeImageGet($arr_podstol["DETAIL_PICTURE"], array('width' => 100, 'height' => 100), BX_RESIZE_IMAGE_PROPORTIONAL, true);
        $arr_podstol["PREVIEW_PICTURE"] = $arr_podstol["PREVIEW_PICTURE"]["src"];
    } else {
        $arr_podstol["PREVIEW_PICTURE"] = NO_PHOTO;
    }
    $allPodstol[] = $arr_podstol;
}

$arResult["PODSTOLS"] = $allPodstol;
if (!$arResult["DETAIL_PICTURE"]["SRC"]) {
    $arResult["DETAIL_PICTURE"]["SRC"] = NO_PHOTO;
}
